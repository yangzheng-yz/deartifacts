...
class FPADDMConvBlock(nn.Module):
...
class FPADDMNet(nn.Module):
    def __init__(
        ...
        if with_time_emb:
            ...
            if multiexposure:
                self.FPAEmbTime = SinusoidalPosEmb(time_dim)
                self.FPAEmbExposure = SinusoidalPosEmb(time_dim)
        ...
        self.l1 = FPADDMConvBlock(channels, half_dim, time_emb_dim=time_dim)
        self.l2 = FPADDMConvBlock(half_dim, dim, time_emb_dim=time_dim)
        self.l3 = FPADDMConvBlock(dim, dim, time_emb_dim=time_dim)
        self.l4 = FPADDMConvBlock(dim, half_dim, time_emb_dim=time_dim)
    ...
    def forward(self, x, time, exposure=None):
...
class MultiFPAGaussianDiffusion(nn.Module):
    def __init__(
        ...
        # for i in range(n_exposures):  # flip xy->hw
        ...
        alphas = 1. - betas
        alphas_cumprod = np.cumprod(alphas, axis=0) # 累积乘积
        ...
        # gamma blur schedule
        # gammas = torch.zeros(size=(n_exposures - 1, self.num_timesteps), device=self.device)
        # for i in range(n_exposures - 1):
    ...
    def roi_patch_modification(self, x_recon, scale=0, eta=0.8):
        ...
        for bb in self.roi_bbs:  # bounding box is of shape [y,x,h,w]
            bb = [int(bb_i / np.power(self.scale_factor, self.n_exposures - scale - 1)) for bb_i in bb]
    ...
    def p_mean_variance(self, x, t, s, clip_denoised: bool):
        ...
        if self.save_interm:
            ...
            utils.save_image(final_img,
                             ...
                             nrow=4)
        # CLIP guidance
        if self.clip_guided_sampling and (self.stop_guidance <= t[0] or s < self.n_exposures - 1) and self.guidance_sub_iters[s] > 0:
            if clip_denoised:
                x_recon.clamp_(-1., 1.)
        ...
        # ROI guided sampling
        elif self.roi_guided_sampling and (s < self.n_exposures-1):
    ...
    def p_sample_loop(self, shape, s):
        ...
        if self.save_interm:
            ...
            utils.save_image(final_img,
                             ...
                             nrow=4)
        if self.sample_limited_t and s < (self.n_exposures-1):
    ...
    def p_sample_via_scale_loop(self, batch_size, img, s, custom_t=None):
        device = self.betas.device
        if custom_t is None:
            total_t = self.num_timesteps_ideal[min(s, self.n_exposures-1)]-1
        else:
            total_t = custom_t
        b = batch_size
    ...
    def sample_via_scale(self, batch_size, img, s, scale_mul=(1, 1), custom_sample=False, custom_img_size_idx=0, custom_t=None, custom_image_size=None):
        ...
        """
        if custom_sample:
            if custom_img_size_idx >= self.n_exposures:  # extrapolate size
                size = self.image_sizes[self.n_exposures-1]
                factor = self.scale_factor ** (custom_img_size_idx + 1 - self.n_exposures)
