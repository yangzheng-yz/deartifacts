...
for epoch in range(start_epoch, epoch_num):  # 100 epochs as an example
    ...
    with torch.no_grad():
        for idx, (val_input_images, val_target_images) in enumerate(val_loader):
            ...
            for j in range(val_input_images.size(0)):
                val_psnr += psnr(val_output_images[j], val_target_images[j]).item()
            pbar_val.set_postfix({"Validation Loss": val_loss_batch.item(), 
                                "Validation PSNR": val_psnr / batch_size})
            # 转换为 numpy 数组并保存
    ...
    writer.add_scalar('Validation Loss', avg_val_loss / len(train_loader), epoch)
    writer.add_scalar('Validation PSNR', avg_val_psnr / len(train_loader), epoch)
